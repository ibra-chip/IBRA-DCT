document.addEventListener('DOMContentLoaded', () => {
	...
	async function loadUsers() { const users = await request('/contacts'); const managerView = isOwner(); const currentUserId = currentUser?.id || currentUser?.sub; const canRemoveUsers = Boolean(managerView && currentUserId); document.querySelector('#users').innerHTML = users.map((user) => `<div class="list-item"><strong>${user.name}</strong><small>${user.role} · ${user.email || 'bez e-maila'} · ${user.phone || 'bez telefona'}${user.siret ? ` · SIRET: ${user.siret}` : ''}${user.company ? ` · Firma: ${user.company}` : ''}${user.employerSiret ? ` · Radi za: ${user.employerCompany || user.company} · SIRET: ${user.employerSiret}` : ''}${managerView && (user.dailyRate || user.hourlyRate) ? ` · Dnevno: ${formatEUR(user.dailyRate)} · Satnica: ${formatEUR(user.hourlyRate)}` : ''}</small>${canRemoveUsers && user.id !== currentUserId ? `<button class="secondary remove-user" data-user="${user.id}" data-name="${escapeHtml(user.name)}" type="button">Ukloni korisnika</button>` : ''}</div>`).join(''); document.querySelectorAll('.remove-user').forEach((button) => button.addEventListener('click', async () => { if (!window.confirm(`Ukloniti korisnika ${button.dataset.name}?`)) return; try { await request(`/users/${button.dataset.user}`, { method: 'DELETE' }); await refreshActiveView(); toast('Korisnik je uklonjen.'); } catch (error) { let message = 'Korisnik nije uklonjen.'; try { message = JSON.parse(error.message).error || message; } catch {} toast(message); } })); const recipient = document.querySelector('#recipient'); recipient.innerHTML = users.filter((user) => user.id !== currentUserId).map((user) => `<option value="${user.id}">${user.name} · ${user.role}</option>`).join(''); }
