document.addEventListener('DOMContentLoaded', () => {
	...
	function renderWorkCalendar(entries, selectedDate, rendezvous = latestRendezvous) {
		...
		for (let day = 1; day <= last.getDate(); day += 1) {
			...
			const rdvButtons = dayRendezvous.map((item) => `<span class="calendar-quick-edit calendar-rendezvous" data-edit-rdv="${item.id}" data-edit-rdv-time="${escapeHtml(item.time || '')}">RDV ${escapeHtml(item.time || '')}<button type="button" class="calendar-icon-btn edit-only" data-edit-rdv="${item.id}" data-edit-rdv-time="${escapeHtml(item.time || '')}" title="Modifier le RDV" aria-label="Modifier le RDV">✎</button></span>`).join('');
