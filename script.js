document.addEventListener('DOMContentLoaded', () => {
	...
	function renderWorkCalendar(entries, selectedDate, rendezvous = latestRendezvous) {
		...
		for (let day = 1; day <= last.getDate(); day += 1) {
			...
			const hoursButton = entry ? `<span class="calendar-quick-edit">${Number(entry.hours || 0).toFixed(1)}h<button type="button" class="calendar-icon-btn" data-edit-hours="${entry.id}" data-edit-hours-value="${Number(entry.hours || 0)}" title="Modifier les heures" aria-label="Modifier les heures">✎</button><button type="button" class="calendar-icon-btn danger" data-delete-hours="${entry.id}" title="Supprimer les heures" aria-label="Supprimer les heures">✕</button></span>` : '';
