document.addEventListener('DOMContentLoaded', () => {
		...
		openQuickEditPopover(anchor, `<strong>Novi RDV / odsustvo</strong><small>${escapeHtml(date)}</small><label>Vreme<input type="time" name="time" required /></label><label>Razlog (opciono)<textarea name="reason"></textarea></label><div class="quick-edit-actions"><button type="button" class="primary" data-qe-save>Sačuvaj</button><button type="button" class="secondary" data-qe-cancel>Otkaži</button></div>`, (popover) => {
			popover.querySelector('[data-qe-save]').addEventListener('click', async () => {
				...
				try {
					...
					closeQuickEditPopover(); await onSaved(); toast('RDV je sačuvan.');
				} catch (error) { toast(`RDV nije sačuvan: ${error.message || 'greška'}`); }
			});
			popover.querySelector('[data-qe-cancel]').addEventListener('click', closeQuickEditPopover);
		});
	}
	...
	Object.assign(runtimeFrench, {
		...
		'RADNIK · MJESEČNI PREGLED': 'OUVRIER · VUE MENSUELLE', 'Detalji radnika': 'Détails de l’ouvrier', 'Mjesec pregleda': 'Mois affiché', 'Učitavanje': 'Chargement', 'KALENDAR': 'CALENDRIER', 'Radni dani i RDV': 'Jours travaillés et rendez-vous', 'ISPLATA': 'PAIEMENT', 'Plata i status': 'Paiement et statut', 'RDV / ODSUSTVO': 'RENDEZ-VOUS / ABSENCES', 'Termini': 'Rendez-vous', 'EVIDENCIJA': 'SUIVI', 'Unosi rada': 'Saisies de temps',
		'Preimenuj': 'Renommer', 'Obriši': 'Supprimer', 'Otvori': 'Ouvrir', 'Otvori praćenje': 'Ouvrir le suivi', 'Izmeni profil': 'Modifier le profil', 'Pošalji link za lozinku': 'Envoyer le lien de mot de passe', 'Obriši radnika': 'Supprimer l’ouvrier', 'Nema dodeljenog chantier-a': 'Aucun chantier attribué', 'Nema zakazanog RDV': 'Aucun rendez-vous prévu',
	...
	function renderWorkerDetailEntries(snapshot) {
		...
		if (!selectedDate) { target.innerHTML = '<small>Klikni jedan dan u kalendaru da vidiš i izmeniš taj unos.</small>'; return; }
	...
	async function markWorkerPaid(userId, month, trigger) {
		...
		try { await request('/payout-requests/mark-paid', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, month }) }); await loadUsers(); renderWorkerDetail(); toast('Plaćanje je označeno.'); }
	...
	workerDetailModal?.addEventListener('click', async (event) => {
		...
		const payoutButton = event.target.closest('[data-payout-status]'); if (payoutButton) { payoutButton.disabled = true; try { await request(`/payout-requests/${encodeURIComponent(payoutButton.dataset.payout)}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: payoutButton.dataset.payoutStatus }) }); await loadUsers(); renderWorkerDetail(); toast(payoutButton.dataset.payoutStatus === 'paid' ? 'Plaćanje je označeno kao završeno.' : 'Status zahteva je promenjen.'); } catch { toast('Status plaćanja nije promenjen.'); } finally { payoutButton.disabled = false; } }
	});
	...
	function renderAutoAnalysis(target, analysis) {
		...
		const imagesSection = analysis.referenceImages?.length ? `<div class="auto-analysis-section auto-analysis-images"><strong>${french ? 'Schémas extraits du document' : 'Šeme izvučene iz dokumenta'}</strong><div class="auto-analysis-image-grid">${analysis.referenceImages.map((img) => `<a href="${escapeHtml(img.dataUrl)}" target="_blank" rel="noopener noreferrer"><img src="${escapeHtml(img.dataUrl)}" alt="${french ? 'Schéma page' : 'Šema strana'} ${escapeHtml(String(img.page))}" loading="lazy" /></a>`).join('')}</div></div>` : '';
		const rulesSection = section(french ? 'Règles et valeurs (extraites du document)' : 'Pravila i vrednosti (izvučeno iz dokumenta)', analysis.extractedRules);
		const referencesSection = analysis.references?.length ? `<div class="auto-analysis-section"><strong>${french ? 'Vidéos et guides de référence' : 'Video i vodiči za referencu'}</strong><ul>${analysis.references.map((ref) => `<li><a href="${escapeHtml(ref.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(ref.title)}</a></li>`).join('')}</ul></div>` : '';
		target.innerHTML = `<div class="list-item auto-analysis"><strong>${french ? 'Analyse automatique' : 'Automatska analiza'} · ${escapeHtml(analysis.workType)}</strong><small>${french ? 'Confiance' : 'Sigurnost'}: ${Math.round(Number(analysis.confidence || 0) * 100)}% · ${escapeHtml(analysis.fileName || '')}</small>${specificAnswer}${rulesSection}${imagesSection}${section(french ? 'Système' : 'Sistem', analysis.systems)}${section(french ? 'Matériaux détectés' : 'Prepoznati materijali', analysis.materials)}${section(french ? 'Comment exécuter' : 'Kako se radi', analysis.howTo)}${section(french ? 'Contrôles' : 'Šta kontrolisati', analysis.controls)}${section(french ? 'Preuves photos' : 'Šta slikati kao dokaz', analysis.evidence)}${section(french ? 'Risques / à confirmer' : 'Rizici / šta potvrditi', analysis.risks)}${referencesSection}</div>`;
		return true;
	}
	...
	async function loadDocuments() { await loadProjectOptions(); const activeProject = currentProjectId(); const documents = (await request('/documents')).filter((item) => !activeProject || item.projectId === activeProject); const canManage = isOwner();
		...
		document.querySelector('#documents').innerHTML = groups.length ? groups.map((group) => `<div class="document-group"><h4>${documentGroupLabels[group.type]} · ${group.items.length}</h4>${group.items.map(renderItem).join('')}</div>`).join('') : '<small>Nema sačuvanih dokumenata.</small>';
		document.querySelectorAll('.rename-document').forEach((button) => button.addEventListener('click', async () => { const item = documents.find((document) => document.id === button.dataset.document); const name = window.prompt('Novo ime dokumenta:', item?.originalName || ''); if (!name?.trim()) return; button.disabled = true; button.setAttribute('aria-busy', 'true'); try { await request(`/documents/${button.dataset.document}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() }) }); await refreshActiveView(); toast('Dokument je preimenovan.'); } catch { toast('Dokument nije preimenovan.'); button.disabled = false; button.removeAttribute('aria-busy'); } })); document.querySelectorAll('.copy-document').forEach((button) => button.addEventListener('click', async () => { button.disabled = true; button.setAttribute('aria-busy', 'true'); try { await request(`/documents/${button.dataset.document}/copy`, { method: 'POST' }); await refreshActiveView(); toast('Kopija dokumenta je dodata.'); } catch { toast('Kopija nije dodata.'); button.disabled = false; button.removeAttribute('aria-busy'); } })); document.querySelectorAll('.delete-document').forEach((button) => button.addEventListener('click', async () => { const item = documents.find((document) => document.id === button.dataset.document); if (!item || !window.confirm(`Obrisati dokument „${item.originalName}“?`)) return; const originalLabel = button.textContent; button.disabled = true; button.textContent = 'Brisanje...'; try { await request(`/documents/${button.dataset.document}`, { method: 'DELETE' }); await refreshActiveView(); toast('Dokument je obrisan.'); } catch { toast('Dokument nije obrisan.'); button.disabled = false; button.textContent = originalLabel; } })); }
	...
	document.querySelector('#budget-form [name="projectId"]')?.addEventListener('change', loadBudget);
		document.querySelector('#purchase-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const file = document.querySelector('#invoice-file').files[0]; const form = event.currentTarget; const body = new FormData(form); if (!file) { toast('Dodaj PDF račun ili opravdanje.'); return; } body.append('invoice', file); const submitButton = form.querySelector('button.primary'); const originalLabel = submitButton?.textContent; if (submitButton) { submitButton.disabled = true; submitButton.textContent = 'Enregistrement...'; submitButton.setAttribute('aria-busy', 'true'); } try { const response = await fetch(`${api}/purchases`, { method:'POST', headers:{Authorization:`Bearer ${token()}`}, body }); if (!response.ok) throw new Error(); form.reset(); await refreshActiveView(); toast('Trošak i PDF faktura su sačuvani; Devis je automatski ažuriran.'); } catch { toast('Trošak nije sačuvan.'); } finally { if (submitButton) { submitButton.disabled = false; submitButton.textContent = originalLabel; submitButton.removeAttribute('aria-busy'); } } });
