document.addEventListener('DOMContentLoaded', () => {
	...
	async function startArMeter(form, status) {
		...
		try {
			...
			overlay.querySelector('#ar-meter-finish').addEventListener('click', async () => {
				...
				status.textContent = `AR mjera: ${value.toFixed(2)} ${unit}. Dodajte/slikajte posao i sacuvajte izvjestaj.`;
				await close();
			});
		} catch (error) {
			overlay.remove();
	...
	document.querySelector('#budget-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const file = document.querySelector('#budget-file').files[0]; const body = new FormData(event.currentTarget); if (file) body.append('file', file); try { const projectId = body.get('projectId'); const response = await fetch(`${api}/projects/${projectId}/budget`, { method:'POST', headers:{Authorization:`Bearer ${token()}`}, body }); if (!response.ok) { const details = await response.text(); throw new Error(`${response.status}: ${details}`); } event.currentTarget.reset(); await loadBudget(); toast('Devis i budžet su sačuvani.'); } catch (error) { toast(`Devis nije sačuvan: ${error.message || 'nepoznata greška'}`); } });
