document.addEventListener('DOMContentLoaded', () => {
	...
	function renderWorkCalendar(entries, selectedDate, rendezvous = latestRendezvous) {
		...
		for (let day = 1; day <= last.getDate(); day += 1) {
			...
			const rdvButtons = dayRendezvous.map((item) => `<span class="calendar-quick-edit calendar-rendezvous" data-edit-rdv="${item.id}" data-edit-rdv-time="${escapeHtml(item.time || '')}">RDV ${escapeHtml(item.time || '')}<button type="button" class="calendar-icon-btn edit-only" data-edit-rdv="${item.id}" data-edit-rdv-time="${escapeHtml(item.time || '')}" title="Modifier le RDV" aria-label="Modifier le RDV">✎</button><button type="button" class="calendar-icon-btn danger" data-delete-rdv="${item.id}" title="Supprimer le RDV" aria-label="Supprimer le RDV">✕</button></span>`).join('');
			...
			const dayDot = entry || dayRendezvous.length ? '<span class="calendar-day-dot" aria-hidden="true"></span>' : '';
			cells += `<div class="calendar-cell${weekend ? ' weekend' : ''}${entry ? ' worked' : ''}${entryStatusClass}${dayRendezvous.length ? ' has-rendezvous' : ''}" data-work-date="${date}" tabindex="0" role="button" title="${escapeHtml(title)}" aria-label="${escapeHtml(`${date}: ${title}`)}"><strong>${day}${dayDot}<span class="calendar-cell-weekday">${weekdayAbbrFr[weekday]}</span></strong>${marker}</div>`;
		}
