...
app.post('/api/auth/register', async (request, response) => {
	...
	const directEmail = String(request.body.email || '').trim().toLowerCase();
	const directPassword = String(request.body.password || '');
	const contact = String(request.body.contact || directEmail || request.body.phone || '').trim();
	...
	if (!contact || (!isEmail && !phone) || !allowedRoles.includes(role) || (role === 'gerant' && !siret) || (role === 'conducteur' && !company) || (role === 'user' && (!/^\S+@\S+\.\S+$/.test(directEmail) || directPassword.length < 10))) return response.status(400).json({ error: 'Valid email or phone, role, and role-specific company details are required' });
	...
	const password = role === 'user' ? directPassword : crypto.randomBytes(9).toString('base64url');
	const user = { id: `user-${Date.now()}`, name, email: role === 'user' ? directEmail : email, phone, role, siret: role === 'gerant' ? siret : '', company: role === 'conducteur' ? company : '', projectIds: ['lot-a'], dailyRate: 0, hourlyRate: 0, passwordHash: await bcrypt.hash(password, 12) };
	if (role === 'user') { data.users.push(user); await writeData(data); return response.status(201).json({ message: 'Account created', email: directEmail, phone: null, direct: true }); }
