...
app.post('/api/auth/register', async (request, response) => {
	...
	if (!contact || (!isEmail && !phone) || !allowedRoles.includes(role) || (['gerant', 'conducteur'].includes(role) && !siret) || (role === 'conducteur' && !company)) return response.status(400).json({ error: 'Valid email or phone, role, and role-specific company details are required' });
