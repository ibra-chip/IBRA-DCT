...
app.post('/api/auth/register', async (request, response) => {
	...
	if (!contact || (!isEmail && !phone) || !allowedRoles.includes(role) || (role === 'gerant' && !siret)) return response.status(400).json({ error: 'Valid email or phone, role, and role-specific company details are required' });
